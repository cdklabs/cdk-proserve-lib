...
export class AosDomainStack extends Stack {
    ...
    constructor(scope: Construct, id: string, props?: StackProps) {
        ...
        this.domainAdminRole = new Role(this, 'OpenSearchAdminRole', {
            assumedBy: new PrincipalWithConditions(new AccountRootPrincipal(), {
                StringLike: {
                    'aws:PrincipalArn': `arn:${Aws.PARTITION}:iam::${Aws.ACCOUNT_ID}:role/Aos*`
                }
            })
        });

        // Create the OpenSearch Domain
        this.domain = new Domain(this, 'AosDomain', {
            version: EngineVersion.OPENSEARCH_2_19,
            fineGrainedAccessControl: {
                masterUserArn: this.domainAdminRole.roleArn
            },
            ebs: {
                volumeSize: 100
            },
        ...
        const provision = new OpenSearchProvisionDomain(
            ...
            {
                domain: this.domain,
                domainAdmin: this.domainAdminRole,
                clusterSettings: {
                    persistent: {
        ...
        const workflow = new OpenSearchWorkflow(this, 'EmbeddingsFlow', {
            domain: this.domain,
            domainAuthentication: this.domainAdminRole,
        ...
        workflow.node.addDependency(this.domain);
